...
class ESPROM:
    ...
    def __init__(self, port = 0):
        # Try to connect to pipe to request access to com port from terminal app
        try:
            self.pipe = os.open(r'\\.\pipe\TERMPIPE', os.O_WRONLY)
            os.write(self.pipe, '<REQUEST>\r\n')
            time.sleep(0.5)
            print 'Pipe opened'
        except:
        ...
        try:
            ...
            if hasattr(self, 'pipe'):
                os.write(self.pipe, '<RELEASE>\r\n')
                os.close(self.pipe)
            print 'Unable to open port'
            sys.exit(1)
 
        time.sleep(0.5)
        # Set DTR low. This is connected via a diode to pull GPIO0 low
        # And to RESET via a .1uf capacitor to give a short reset pulse
        # and hold down GPIO0 after reset to enable bootloader mode
        self._port.setDTR(1)
        # ESP8266 seems to read GPIO0 about 15ms after bootup
        # After that, it seems to revert to output state 
        time.sleep(0.016)
        self._port.setDTR(0)

    """ Close the com port, release it back to serial terminal """ 
    def close_com(self):
        if hasattr(self, 'pipe'):
            os.write(self.pipe, '<RELEASE>\r\n')
            os.close(self.pipe)
